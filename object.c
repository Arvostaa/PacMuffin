...
void drawCookiePosition(int *cookieX, int *cookieY){
	...
	*cookieX = rand()%11;
	*cookieY = rand()%15;
	if(map[*cookieX][*cookieY].type == FIELD_WALL){
		while(map[*cookieX][*cookieY].type == FIELD_WALL){
			*cookieX = rand()%11;
			*cookieY = rand()%15;
		}
	}

	*cookieX = *cookieX*TILE_SIZE;
	*cookieY = *cookieY*TILE_SIZE;
	//printf("%d - cookieX, %d - cookieY, %d - fieldType\n", cookieX, cookieY, map[cookieX/TILE_SIZE][cookieY/TILE_SIZE].type);
}

void setCookiePosition(cookie_s *cookieList, int *cookiePosX, int *cookiePosY){ // once i a while it doesn't appear !
	cookie_s *wsk;
	wsk = cookieList;
